<?php		
	...
	//Group Discount
	if( class_exists('Wp_Travel_Engine_Group_Discount') && isset($wp_travel_engine_setting['group']['discount']) )
    {
	...
	////////////
	if( class_exists('Wp_Travel_Engine_Group_Discount') && isset( $wp_travel_engine_setting['group']['discount']) && isset( $_POST['child-travelers'] ) && $_POST['child-travelers']!='0' )
	{ 
	...
	if( class_exists('Wp_Travel_Engine_Group_Discount') && isset($wp_travel_engine_setting['group']['discount']) && isset( $_POST['child-travelers'] ) && $_POST['child-travelers']!='0' && isset($per_traveler) && $per_traveler !='' && isset($per_child) && $per_child !='' && isset($child_cost) && $child_cost != '')
    {
		...
		<div class="wp-travel-engine-order-form-wrapper">
			...
			<div class="wp-travel-engine-order-right-column">
				...
				<ul class="trip-property">
					...
					?>
					<?php
					if( class_exists( 'Wp_Travel_Engine_Group_Discount' ) && isset( $wp_travel_engine_setting['group']['discount'] ) && isset( $_POST['child-travelers'] ) && $_POST['child-travelers']!='0' && isset($per_child) && $per_child !='' )
					{
						if( class_exists( 'Wte_Trip_Currency_Converter_Init' ) )
				        { 
					...
					{	?>
						<li class="cart-trip-total-price"><span style="width: auto;"><?php _e('Extra Service(s)','wp-travel-engine');?></span>
						<div class="extra-service">
						<?php 
						$total_extra_cost = 0;
    	        		foreach ($_POST['extra_service'] as $key => $value) {
    	        			if( isset( $_POST['extra_service'][$key] ) && $_POST['extra_service'][$key]!='' && isset( $_POST['extra_service_name'][$key] ) && $_POST['extra_service_name'][$key]!= '' )
    	        			{
								echo '<span class="extra-service-name">'.$wp_travel_engine_setting['extra_service'][$key].': </span>';
								echo '<span class="extra-service-cost">'.$_POST['extra_service'][$key].' X '.esc_attr($currency).esc_attr( $obj->wp_travel_engine_price_format( $_POST['extra_service_name'][$key] ) ).esc_attr($code).'</span>';
					...
					{ 
						...
						<li class="cart-trip-total-price"><span style="width: auto;"><?php _e('Total Remaining Payment:  ','wp-travel-engine');?></span><?php echo '<span class="currency" style="width: auto;">'.esc_attr($currency).'</span><span class="cart-trip-total-price-holder" style="width: auto;">'.esc_attr( $obj->wp_travel_engine_price_format( $tcost-$deposit_cost ) ).'</span><span class="currency-code" style="width: auto;">'.esc_attr( ' '.$code ).'</span>';?>
				...
				<div class="wp-travel-engine-billing-details-wrapper">
					...
					<div class='wp-travel-engine-billing-details-field-wrap'>
							...
							switch ($key) {
								...
								case 'address':?> 
								<label for="<?php echo esc_attr( $key );?>"><?php _e('Phone No','wp-travel-engine');?><span class="required">*</span></label>
								<input type="<?php echo $value['type'];?>" name="wp_travel_engine_booking_setting[place_order][booking][<?php echo esc_attr( $key );?>]" id="<?php echo esc_attr( $key );?>" <?php if( $value['required'] == '1' ) { echo 'required';   } ?>>	
								<?php
								break;
