...
class Travel_Agency_Companion_Public {
	...
	public function enqueue_styles() {
        ...
        wp_enqueue_style( 'owl-carousel', plugin_dir_url( __FILE__ ) . 'css/owl.carousel.min.css', null, '2.2.1' );
	...
	public function enqueue_scripts() {
		...
		{
        	wp_enqueue_script( 'owl-carousel', plugin_dir_url( __FILE__ ) . 'js/owl.carousel.min.js', array( 'jquery' ), '2.2.1', true );
		}	
    ...
    public function front_page_sections(){
		...
		for($i=1;$i<=5;$i++)
		{
			 $package      = get_theme_mod( 'ed_deal_section_'.$i, true );
			  if($package) array_push( $sections, 'package_'.$i );
		}
